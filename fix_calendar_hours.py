"""
Fix: календарь показывает нерабочие часы.
Запуск: python fix_calendar_hours.py
"""
import shutil
...
def backup():
    ...
    backup_dir = BASE / f"backup_fix_hours_{timestamp}"
...
NEW_CALENDAR_ROUTE = '''@app.route('/admin/calendar')
def admin_calendar():
    ...
    from core.storage.schedule import (
        get_all_work_hours, is_day_off, _time_to_minutes
    )
